	...
	rui_validate.prototype = {
		...
		initForm: function (user_options) {
            ...
            */
		    rui_Validate_options.FormObj.find("*[verify]").focus(function () {
		        ...
		        var _thisIsNot = $(this).attr("isNot"); 
		        if(_thisIsNot==undefined || _thisIsNot == "false"){
		    ...
		    rui_Validate_options.FormObj.find("*[verify]").blur(function () {
		        ...
		        if (theUrl != "" && theName != "" && theValue != "") {
		        	...
		        	var theIsNot = $(this).attr("isNot") == undefined ? false : $(this).attr("isNot") ;
		            if (rui_Validate_options.BlurChange&&theIsNot&&theIsNot!='false') {  //blur就开始验证表单内容
		                var _thisMsg = $(this).attr("msg") == undefined ? "" : $(this).attr("msg");
		                if (_thisMsg != undefined && _thisMsg != "" && theValue != "" ) { 
		                	...
		                	var theCheckResult = { statue: "Error_validate", obj: $(this), msg: _thisMsg };   
		                    _this.ShowTips(rui_Validate_options, $(this), theCheckResult.msg, theCheckResult.statue);  //展现结果 
		                } 
		            }else{ 
		            	_this.clearShowTips(rui_Validate_options, $(this));
            ...
            */
		    rui_Validate_options.FormObj.find("." + rui_Validate_options.SubBtn + " , input[type='submit']").eq(0).click(function () {
		        ...
		        //提交表单进行验证 
		        var _theState = rui_Validate_options.FormObj.attr("state");
		        if (_theState == undefined || _theState == "true") {
		            if (rui_Validate_options.Frequent != undefined) {
		                //频繁点击
		                rui_Validate_options.FormObj.attr("state", false);
		            }
		            //提交表单进行验证
		    ...
		    rui_Validate_options.FormObj.submit(function(){
		        var _theState = rui_Validate_options.FormObj.attr("state");
		        if (_theState == undefined || _theState == "true") {
		            if (rui_Validate_options.Frequent != undefined) {
                        //频繁点击
        ...
        */
		ShowTips: function (rui_Validate_options, obj, msg, state) { 
		    ...
		    switch (rui_Validate_options.ShowTip) {
				...
				case "alert_layer": 
		            if (state == "Error_validate") {
		                var _theMsg = obj.attr("msg") == undefined ? "" : obj.attr("msg"); 
						msg = msg || _theMsg;
		...
		ShowTips_Bubble: function (rui_Validate_options, obj, msg, state) {
		    ...
		    if (_obj_p.find("." + rui_Validate_options.FormIdName + "_Bubble_ts_msg").length > 0) {  //是否已有提示信息 
		        ...
		        if (state == "Error_validate") {
		            ...
		            var _style = " position: absolute;width:auto; max-width:" + (_obj_width - 15) + ";  background-color:#fff; padding:0px 15px;height:" + (_new_height - 2) + "px;line-height:" + (_new_height - 2) + "px;border:1px solid #ddd; margin-left:" + (_obj_width + 35) + "px; float:left; box-shadow:2px 2px 2px #ddd; background-color:#fff; border-radius:3px;z-index:2; color:red";
		            ...
		            var _str = "<div class='" + rui_Validate_options.FormIdName + "_Bubble_ts_msg bgc_wite ts_msg' style='" + _style + "'><i class='arrow' style='" + arrow_style + "'><b style='" + arrow_innerstyle + "'></b></i><span class='msg'>" + msg + "</span></div>";
        ...
        */ 
		ShowTips_IconText: function (rui_Validate_options, obj, msg, state) {
		    ...
		    if (_obj_p.find("." + rui_Validate_options.FormIdName + "_Bubble_ts_msg").length > 0) {
		        ...
		        var _style = "width:auto; max-width:" + (_obj_width - 15) + "; height:" + (_obj_height -2) + "px;line-height:" + (_obj_height - 2) + "px;  margin-left:5px; float:left; z-index:2; color:red";
        ...
        */
		ShowTips_Text: function (rui_Validate_options, obj, msg, state) {
		    ...
		    if (_obj_p.find("." + rui_Validate_options.FormIdName + "_Bubble_ts_msg").length > 0) {
				 if (state != "Correct_validate") {
		       		 _obj_p.find("." + rui_Validate_options.FormIdName + "_Bubble_ts_msg").empty().text(msg).show();
				 }else{
					 _obj_p.find("." + rui_Validate_options.FormIdName + "_Bubble_ts_msg").empty().text(msg).hide();
				 }
		    }
		    else {
		        var _style = "width:auto; max-width:" + (_obj_width - 15) + "; height:" + (_obj_height - 2) + "px;line-height:" + (_obj_height - 2) + "px;  margin-left:5px; float:left; z-index:2; color:red";
        ...
        */
		ShowTips_Icon: function (rui_Validate_options, obj, msg, state) {
		    ...
		    if (_obj_p.find("." + rui_Validate_options.FormIdName + "_Bubble_ts_msg").length > 0) {
		        ...
		        var _style = "position: absolute;width:auto; max-width:" + (_obj_width - 15) + "; height:" + (_new_height - 2) + "px;line-height:" + (_new_height - 2) + "px;  margin-left:" + (_obj_width + 5) + "px; float:left; color:red;  margin-top:-" + _obj_height + "px; z-index:2;";
        ...
        */ 
		Subvalidation: function (rui_Validate_options, obj) {
		    ...
		    if (_result_cont.length < 1) {
		        ...
		        //存在链接提交
		        if (theFormObjUrl != "" && theFormObjUrl != undefined) {  //如果提交是以 AJAX 提交 则执行一下 回调函数 
		        	if (theCollBack == "") {
			            theCollBack = formSuccess;
			        } 
		        	//url:form提交数据的地址 
		        	//type:form提交的方式(method:post/get) 
		        	//beforesubmit:提交前执行的回调函数 
		        	//success:提交出错后执行的回调函数  
		        	//dataType:服务器返回数据类型 Null为不限制，json ， script ， xml  ， html ， text
					...
					rui_Validate_options.FormObj.ajaxSubmit(JQ_FormOptions);
					obj.prop("disabled", false);
					
		            return false;
		        }else{   //如果提交是以 默认form 提交 则执行一下 回调函数
		        	 if (theCollBack != "") {
			            theCollBack();
			        }
		        }
